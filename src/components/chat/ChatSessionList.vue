...
<template>
  <div class="session-list">
    ...
    <div class="session-search-wrapper">
      ...
      <input v-model="search" type="text" placeholder="Search..." class="session-search" />
    </div>
    ...
    <div class="session-items">
      ...
      <div v-for="session in filtered" :key="session.id" :class="['session-item', { active: session.id === activeId }]"
        @click="emit('select', session.id)">
        <div class="session-item-content">
          <p class="session-title">{{ session.title }}</p>
          <div class="session-meta">
            <span class="session-time">{{ relativeTime(session.updatedAt) }}</span>
            <span v-if="session.tenantId && session.tenantId === activeTenantId" class="tenant-dot"
              title="Linked to current tenant" />
          </div>
        </div>
        ...
        <template v-else>
          <button type="button" class="delete-btn" @click="confirmDelete(session.id, $event)"
