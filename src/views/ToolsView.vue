  ...
  <AppLayout>
    <div class="px-4 md:px-6 lg:px-8 py-6 max-w-5xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <div class="flex gap-2 flex-wrap items-center">
          <button @click="selectedNamespace = ''"
            class="text-xs font-medium px-3 py-1.5 rounded-full border-0 transition-colors cursor-pointer" :style="!selectedNamespace
              ? 'background: var(--color-primary-subtle, rgba(99,102,241,.12)); color: var(--color-primary, #6366f1);'
              : 'background: var(--bg-muted); color: var(--text-secondary);'">All</button>
          <button v-for="ns in namespaces" :key="ns" @click="selectedNamespace = ns === selectedNamespace ? '' : ns"
            ...
            :style="selectedNamespace === ns
              ...
              : 'background: var(--bg-muted); color: var(--text-secondary);'">{{ ns }}</button>
        </div>
      </div>
      ...
      <div v-if="toolStore.isLoading" class="flex flex-col gap-2">
        <div v-for="i in 6" :key="i" class="rounded-xl border px-5 py-4 flex flex-col gap-2"
            ...
            <div v-if="expandedTool === tool.name" class="border-t" style="border-color: var(--border-default);">
              ...
              <div class="px-5 pt-3.5 pb-1 flex items-center gap-2">
                <span class="text-[10px] uppercase tracking-wide font-semibold"
                  style="color: var(--text-tertiary);">Full
                  name</span>
              ...
              <div v-if="getParams(tool).length" class="px-5 pb-4">
                <p class="text-[10px] uppercase tracking-wide font-semibold mb-2" style="color: var(--text-tertiary);">
                  Parameters</p>
                <div class="rounded-lg overflow-hidden border text-xs" style="border-color: var(--border-default);">
                  ...
                  <div v-for="(param, idx) in getParams(tool)" :key="param.name"
                    ...
                    <code class="font-mono font-semibold text-[11px] break-all" style="color: var(--text-primary);">{{
                param.name }}</code>
