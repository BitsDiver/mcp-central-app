...
function messagesToOllama(messages: ChatMessage[], systemPrompt?: string): OllamaMessage[] {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === 'user') {
      ...
      for (const tc of msg.toolCalls) {
        result.push({
          role: 'tool',
          content: tc.result !== undefined ? JSON.stringify(tc.result) : (tc.error ?? ''),
          tool_call_id: tc.id,
        })
      }
    }
...
export function useOllama() {
  ...
  async function generate(opts: {
    ...
    const { ollamaUrl, model, contextSize, systemPrompt, messages, tools, onToken, onToolCall, onDone, onError } = opts
            ...
            if (chunk.done) {
              if (pendingToolCalls.length > 0) {
                ...
                for (const tc of pendingToolCalls) {
                  ...
                  const toolName = parts.slice(1).join('_') || tc.function.name

                  try {
                    const result = await api.callTool(namespace, toolName, tc.function.arguments)
